...
modulatedData = qammod(data, 4); % QPSK modulation 
...
for k = 1:length(snr_dB) 
 ...
 snr = 10^(snr_dB(k)/10); % Convert SNR from dB to linear scale
...
title('BER Performance of OFDM over Multipath Fading Channel');
xlabel('SNR (dB)');
ylabel('BER');
grid on;